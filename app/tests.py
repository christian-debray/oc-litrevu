    ...
    def test_find_tickets_followed_by_user(self):
        ...
        for test_data in expectations:
            ...
            self.assertListEqual(
                ticket_ids,
                test_data['expected'],
                f"Results don't match for user #{user.pk} {user}, expected {test_data["expected"]}, found {ticket_ids}")
...
class ReviewUserManagerTestCase(TestCase):
    ...
    def test_find_reviews_followed_by_user(self):
        ...
        for test_data in expectations:
            user = User.objects.get(pk=test_data['user.pk'])
            # reviews_1 = Review.with_user_manager.own(user)
            # reviews_2 = Review.with_user_manager.followed(user)
            # review_ids = sorted([x.pk for x in chain(reviews_1, reviews_2)])
